...
// Public read of site config — used by the public site (title/favicon/brand).
// One singleton row. `inviteKey` is stripped: it gates signups, never leaks.
export const get = query({
  args: {},
  handler: async (ctx) => {
    const s = await ctx.db.query("siteSettings").first();
    if (!s) return null;
    const { inviteKey: _inviteKey, ...safe } = s;
    return safe;
  },
});

// Admin-only full read (Settings UI needs the invite key to display/edit it).
...
const FIELDS = {
  ...
  // Admin-menu signup policy: "open" | "invite" (see convex/auth.ts).
